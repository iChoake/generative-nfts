...
function GeneratePatterns(traits) {
    ...
    if (traits.pattern == 'Egg pattern') var patternCount = DoOnceOrMore(GenerateEgg, probRe, patternCount, textureLayer);
    if (traits.pattern == 'Vertical pattern') var patternCount = DoOnceOrMore(GenerateRectangleVertical, probRe, patternCount, textureLayer);
    if (traits.pattern == 'Horizontal pattern') var patternCount = DoOnceOrMore(GenerateRectangleHorizontal, probRe, patternCount, textureLayer);
    if (traits.pattern == 'Sin pattern') var patternCount = DoOnceOrMore(GenerateSinPattern, probRe, patternCount, textureLayer);
    if (traits.pattern == 'Cookie pattern') var patternCount = DoOnceOrMore(GenerateCookie, probRe, patternCount, textureLayer);
